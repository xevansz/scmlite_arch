      ...
      <div className="max-w-7xl mx-auto px-8 py-8">
        <div className="mb-8">
          ...
          <p className="text-[var(--color-text-muted)]">
            View and filter real-time device telemetry data
          </p>
        </div>
        ...
        <div className="bg-[var(--color-surface)] border border-[var(--color-secondary)] rounded p-6 mb-6">
          ...
          <div className="flex flex-col md:flex-row gap-4 items-end">
            <div className="flex-1">
              <label
                ...
                className="block text-[var(--color-text)] mb-2 text-sm"
              >
          ...
          {!showAllDevices && filterDeviceId && (
            <div className="mt-4 p-3 bg-[var(--color-background)] rounded">
              <p className="text-[var(--color-text-muted)] text-sm">
                Currently showing data for Device ID:{" "}
        ...
        {!showAllDevices && filterDeviceId && (
          <div className="bg-[var(--color-surface)] border border-[var(--color-secondary)] rounded overflow-hidden mb-6">
            <div className="px-6 py-4 border-b border-[var(--color-secondary)]">
              <h2 className="text-[var(--color-text)] text-lg font-semibold">
                Shipments for Device {filterDeviceId}
              </h2>
            </div>
            {loadingShipments ? (
              <div className="text-center py-8">
                <div className="inline-block w-6 h-6 border-4 border-[var(--color-primary)] border-t-transparent rounded-full animate-spin"></div>
              ...
              <div className="text-center py-8">
                <p className="text-[var(--color-text-muted)]">
                  No shipments found for this device ID.
                </p>
              </div>
            ) : (
          ...
          <div className="bg-[#0f1729] border border-[#1e2a45] rounded overflow-hidden">
            ...
            {deviceData.length === 0 ? (
              <div className="text-center py-12">
                ...
                {!showAllDevices && (
                  <p className="text-[#8b92a7] mt-2">
                    Try a different device ID or click "Show All" to view all
                    devices.
                  </p>
                )}
              </div>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full">
                    <thead className="bg-[#151d30]">
                      <tr>
                        <th className="px-6 py-4 text-left text-[#8b92a7]">
                          Device ID
                        </th>
                        <th className="px-6 py-4 text-left text-[#8b92a7]">
                          Battery Level
                    ...
                    <tbody className="divide-y divide-[#1e2a45]">
                      {deviceData.map((data) => (
                        <tr
                          key={data._id}
                          className="hover:bg-[#151d30] transition-colors"
                        >
